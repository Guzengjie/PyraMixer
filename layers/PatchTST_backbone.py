...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len],(32, 7, 96)
        # norm
        if self.revin:
            z = z.permute(0, 2, 1)
        ...
        z = z.unfold(dimension=-1, size=self.patch_len, step=self.stride)  # z: [bs x nvars x patch_num x patch_len](32, 7, )
        z = z.permute(0, 1, 3, 2)  # z: [bs x nvars x patch_len x patch_num]

        # model

        # print(z.shape, "1")  ,  torch.Size([32, 7, 48, 8])
        z = self.backbone(z)  # z: [bs x nvars x d_model x patch_num]
        # print(z.shape, "2"),  torch.Size([32, 7, 256, 8])

        z = self.head(z)  # z: [bs x nvars x target_window]
        # print(z.shape, "3")  , torch.Size([32, 7, 96])
...
class TSTEncoderLayer(nn.Module):
    ...
    def forward(self, src: Tensor, prev: Optional[Tensor] = None, key_padding_mask: Optional[Tensor] = None,
        ...
        ## Multi-Head attention
        # print(src.shape, "5")  # torch.Size([224, 8, 256])
        # exit()
        if self.res_attention:
